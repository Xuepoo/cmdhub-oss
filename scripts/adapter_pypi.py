...
def fetch(out_path: Path, proxy: str, top: int, rate: float) -> None:
    ...
    print(f"[pypi] checking {len(names)} packages for Environment::Console ...", flush=True)
    ...
    for i, name in enumerate(names):
        ...
        try:
            ...
            if not any(c.startswith("Environment :: Console") for c in classifiers):
                continue
            desc = (info.get("summary") or "").strip()
            records.append({
                "app_id": f"org.pypi.{name}",
                "name": name,
                "cmd_path": name,
            ...
            pass
        if (i + 1) % 500 == 0:
            print(f"[pypi] {i+1}/{len(names)} checked, {len(records)} CLIs", flush=True)
...
def main() -> None:
    ...
    a = ap.parse_args()
    fetch(a.out, a.proxy, a.top, a.rate)
